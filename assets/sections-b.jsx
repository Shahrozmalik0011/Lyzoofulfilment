      ...
      <div className="wrap">
        ...
        <div className="grid cols-3 reveal" style={{marginTop:44}}>
          {PRICE_CARDS.map(card => (
            <div key={card.label} className="card" style={{padding:'28px',display:'flex',flexDirection:'column',gap:0}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20}}>
                <span style={{width:42,height:42,borderRadius:12,background:card.color,display:'inline-flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'#fff'}}>{I[card.icon]({style:{width:22,height:22}})}</span>
                <span style={{fontWeight:800,fontSize:'1.05rem',color:'var(--ink)'}}>{card.label}</span>
              </div>
              <div style={{display:'grid',gap:0}}>
                {card.items.map((item,i) => (
                  <div key={item.name} style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',gap:12,padding:'12px 0',borderBottom:i<card.items.length-1?'1px solid var(--line)':'none'}}>
                    <span style={{fontSize:'.93rem',color:'var(--body)'}}>{item.name}</span>
                    <span style={{display:'flex',flexDirection:'column',alignItems:'flex-end',flexShrink:0}}>
                      <span style={{fontWeight:800,color:'var(--ink)',fontSize:'1rem'}}>{item.price}</span>
                      <span style={{fontSize:'.75rem',color:'var(--muted)'}}>{item.unit}</span>
                    </span>
                  </div>
                ))}
              </div>
